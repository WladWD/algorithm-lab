...
using namespace algorithms::graphs::kruskal_mst;
...
TEST(KruskalMst, EmptyGraphN0) {
    auto res = kruskal_mst(0, {});
...
TEST(KruskalMst, SingleVertex) {
    auto res = kruskal_mst(1, {});
...
TEST(KruskalMst, ConnectedDemoGraph) {
    // 0--1 (1)
    // 1--2 (2)
    // 0--2 (10)
    // 2--3 (3)
...
TEST(KruskalMst, DisconnectedGraphReturnsForest) {
    // Component A: 0-1-2
    // Component B: 3-4
    std::vector<Edge> edges = {
        {0, 1, 5},
...
TEST(KruskalMst, ParallelEdgesPickLighter) {
    std::vector<Edge> edges = {
        {0, 1, 10},
        {0, 1, 2},
        {1, 2, 3},
    ...
    auto res = kruskal_mst(3, edges);
    EXPECT_EQ(res.components, 1);
    EXPECT_EQ(static_cast<int>(res.edges.size()), 2);
    EXPECT_EQ(res.total_weight, 2 + 3);
}

TEST(KruskalMst, SelfLoopsAndInvalidEndpointsIgnored) {
    std::vector<Edge> edges = {
        {0, 0, -100}, // self-loop
        {0, 1, 4},
        {-1, 1, 1},  // invalid
...
TEST(KruskalMst, NegativeWeightsAreHandled) {
    std::vector<Edge> edges = {
        {0, 1, -5},
        {1, 2, -1},
        {0, 2, 10},
