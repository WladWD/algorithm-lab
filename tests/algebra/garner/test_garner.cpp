...
static uint64_t brute_force_crt(const std::vector<uint64_t>& a, const std::vector<uint64_t>& m) {
    if (a.size() != m.size()) throw std::invalid_argument("size mismatch");
    uint64_t x = 0;
    uint64_t step = 1;
    for (size_t i = 0; i < a.size(); ++i) {
        ...
        if (!found) throw std::runtime_error("no solution in brute_force_crt");
    }
    return x;
}
...
TEST(GarnerTest, RandomSmallModuli) {
    ...
    std::vector<std::vector<uint64_t>> mod_sets = {
        {2,3,5}, {3,7,11}, {5,7,9}, {4,9,25} // note: some are not pairwise-coprime; will skip invalid ones
    };
    for (auto &ms : mod_sets) {
        ...
        for (uint64_t r = 0; r < 5; ++r) {
            ...
            for (size_t i = 0; i < k && ok; ++i) for (size_t j = i+1; j < k; ++j) if (std::gcd(ms[i], ms[j]) != 1) ok = false;
            if (!ok) continue;
