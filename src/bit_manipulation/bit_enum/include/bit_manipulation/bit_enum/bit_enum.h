...
constexpr T next_same_popcount(T x) noexcept {
...
template <std::unsigned_integral T>
std::vector<T> collect_submasks(T mask) {
    std::vector<T> result;
...
///   gray(n) = n ^ (n >> 1)
template <std::unsigned_integral T>
constexpr T gray_encode(T n) noexcept {
    return n ^ (n >> 1);
}
...
template <std::unsigned_integral T>
constexpr T gray_decode(T g) noexcept {
    T n = g;
