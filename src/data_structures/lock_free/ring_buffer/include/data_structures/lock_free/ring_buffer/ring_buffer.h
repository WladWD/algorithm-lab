...
template <typename T> class SpscRingBuffer {
  public:
    explicit SpscRingBuffer(size_t capacity)
        : capacity_(next_pow2(std::max<size_t>(2, capacity))), mask_(capacity_ - 1),
          buffer_(capacity_) {
  ...
  public:
    ...
    bool try_dequeue(T& out) {
        ...
        if (seq == cid + 1) {
            ...
            slot.seq.store(
                cid + capacity_,
                std::memory_order_release); // make slot available; set to next expected ticket
...
template <typename T> class MpmcRingBuffer {
  public:
    static_assert(std::is_nothrow_move_constructible_v<T> ||
                      std::is_nothrow_copy_constructible_v<T>,
                  "MpmcRingBuffer requires T to be nothrow-move-constructible or "
        ...
        while (true) {
            ...
            if (dif == 0) {
                if (head_.compare_exchange_weak(pos, pos + 1, std::memory_order_relaxed,
                                                std::memory_order_relaxed)) {
                    T* ptr =
                        std::launder(reinterpret_cast<T*>(static_cast<void*>(slot.storage.data())));
                    out = std::move(*ptr);
                    ptr->~T();
