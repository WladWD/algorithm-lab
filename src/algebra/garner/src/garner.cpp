...
uint64_t modinv(uint64_t a, uint64_t mod) {
    if (mod == 0) throw std::invalid_argument("modulus must be > 0");
    int64_t ai = static_cast<int64_t>(a % mod);
    int64_t mi = static_cast<int64_t>(mod);
    auto [g, x, y] = ext_gcd(ai, mi);
    if (g != 1) throw std::invalid_argument("modular inverse does not exist (gcd != 1)");
    int64_t inv = x % mi;
    if (inv < 0) inv += mi;
...
std::vector<uint64_t> garner_mixed(std::vector<uint64_t> a, const std::vector<uint64_t>& m) {
    size_t k = a.size();
    if (k != m.size()) throw std::invalid_argument("garner_mixed: sizes of a and m must match");
    // trivial cases
    if (k == 0) return {};
...
uint64_t garner_mod(const std::vector<uint64_t>& a, const std::vector<uint64_t>& m, uint64_t MOD) {
    if (MOD == 0) throw std::invalid_argument("garner_mod: MOD must be > 0");
    ...
    if (result < 0) result += static_cast<__int128>(MOD);
