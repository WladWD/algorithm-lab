...
uint64_t mul_mod_u128(uint64_t a, uint64_t b, uint64_t mod) {
    if (mod == 0)
        throw std::invalid_argument("mul_mod: modulus must be > 0");
...
uint64_t powmod_bigexp(uint64_t base, const std::string& exp_dec, uint64_t mod) {
    ...
    for (char ch : exp_dec) {
        if (ch < '0' || ch > '9')
            throw std::invalid_argument("exp_dec must be decimal digits");
        int d = ch - '0';
        res = powmod_u128(res, 10, mod);
        if (d)
            res = mul_mod_u128(res, powmod_u128(base, d, mod), mod);
    }
    return res;
}
} // namespace algebra::powmod
